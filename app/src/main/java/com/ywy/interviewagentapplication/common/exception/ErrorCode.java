 ...
 * 业务错误码枚举
 * 设计原因：
 *   1. 按业务模块分段分配 code（1xxx 通用、2xxx 简历、3xxx 面试...）
 *   2. 每个错误码携带默认 message，用于在抛出异常时覆盖常规报错信息
 *   3. 前端根据 code 做国际化或不同的 UI 提示
 * 被依赖方：BusinessException（构造时引用）、Result.error(ErrorCode)（直接作为参数）、
 *           GlobalExceptionHandler（全局处理中引用通用错误码）
 * 字段：
 *   code    — 4/5 位数字错误码，按模块分段
 *   message — 默认错误描述（中文），可在 BusinessException 构造时覆盖
 */
@Getter
@AllArgsConstructor
public enum ErrorCode {
    ...
    /** 错误码：4/5位数字，按模块分段（1xxx/2xxx/.../11xxx） */
    private final Integer code;
    /** 默认错误描述（中文），可在 BusinessException 构造时覆盖 */
